...
{
  // Load image from file
  //std::string filename = "imgs/canva.png";
  // cv::Mat img = cv::imread(filename.c_str());
  // if(img.empty()) {
  //   throw std::runtime_error("Failed to open the file " + filename);
  // }
  ...
  cv::Mat green_mask;
  //cv::inRange(hsv_img, cv::Scalar(40, 60, 119), cv::Scalar(88, 249, 255), green_mask);
  cv::inRange(hsv_img, cv::Scalar(32, 65, 45), cv::Scalar(89,215 , 200), green_mask);
 // cv::imshow("GREEN_filter", green_mask);
    ...
    if (contours[i].size() > 200) 
  {
    ...
    std::cout << (i+1) << ") Contour size: " << contours[i].size() << std::endl;
    approxPolyDP(contours[i], approx_curve, 4, true);
    contours_approx = {approx_curve};
...
int main(int argc, char* argv[])
{
  ...
  imwrite("abc.jpg",unwarped_img);
  processNumbers(processRGB(unwarped_img));
  return 0;
}